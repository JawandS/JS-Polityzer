...
import os
import datetime # for the logging file name
...
def create_logger(filename="logfile.log"):
    ...
    randomizer = datetime.datetime.now().strftime(timestampFmt) + "."
...
def get_javascript_database_file():
    """returns the javascript database file handler to write downloaded javascript entries"""

    if not os.path.isdir(config.DATABASE_FOLDER):
        os.mkdir(config.DATABASE_FOLDER)

    if os.path.isfile(config.JAVASCRIPT_DATABASE_FILE):
        db_file = open(config.JAVASCRIPT_DATABASE_FILE, "a")
        writer = csv.writer(db_file, delimiter=",")
    else:
        db_file = open(config.JAVASCRIPT_DATABASE_FILE, "w")
        writer = csv.writer(db_file, delimiter=",")
        writer.writerow(["name", "url", "filepath", "depth"])
    return writer
    ...
    def load_candidates():
        ...
        with open(config.CANDIDATE_OFFICE_WEBSITE) as f:
            ...
            for candidate in reader:
                candidate_name = candidate["fec_name"].replace(",", "").replace(" ", "").replace("/", "")
